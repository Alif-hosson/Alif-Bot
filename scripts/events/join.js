...
.then(response => {
    ...
    return api.sendMessage("চলে এসেছি আমি আলিফের পিচ্চ বট তোমাদের মাজে-😚😉🥀", event.threadID, () => api.sendMessage({ body: `${global.config.BOTNAME} 𝗖𝗼𝗻𝗻𝗲𝗰𝘁𝗲𝗱 ↤
...
✿ 𝗪𝗣: 01615623**99`, attachment: fs.createReadStream(gifPath)}, threadID));
})
.catch(error => {
    console.error(error);
});
